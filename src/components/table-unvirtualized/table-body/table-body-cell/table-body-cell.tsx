...
const TableBodyCell = forwardRef<HTMLTableCellElement, CellProps>(
    ({ children, align = 'left', isTextContent = true, className, colSpan, style }, ref) => {
        return (
            <td className={classNames('h-16 pr-6 first:pl-6', className)} ref={ref} style={style} colSpan={colSpan}>
                {isTextContent ? (
                    <div className={classNames('paragraph-100', cellAlignStyles[align])}>{children}</div>
                ) : (
                    children
                )}
            </td>
